rootf= path_to_folder + '/Example_diffRBM/'
...
def align_to_seed(seqs_seed, lea, lepmin, lepmax, seqs_new, weights=None):
    ...
    if weights is None:
        alignc_new = eng.balign_prots_seedhmm(seqs_seed, lea, lepmin, lepmax, seqs_new)
    else:
        alignc_new = eng.balign_prots_seedhmm(seqs_seed, lea, lepmin, lepmax, seqs_new, weights) # lea -> final length, lepmin to lepmax -> length included in the profile itself
    eng.quit()
...
np.savetxt(rootf + '/Align_utils/aligned_prot.txt', seqs_new_n, fmt='%i')
